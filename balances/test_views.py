...
from balances.factories import BalanceFactory, BalanceLogFactory, WithdrawFactory
...
class WithdrawListCreateViewTest(BaseAPITestCase):

    def setUp(self):
        super(WithdrawListCreateViewTest, self).setUp()
        self.url = reverse('balances:withdraw')
        self.logs = BalanceLogFactory.create_batch(size=3, organizer=self.organizer,
                                                   status='available')
        self.withdraw = WithdrawFactory(organizer=self.organizer, logs=self.logs,
                                        amount=300000)
